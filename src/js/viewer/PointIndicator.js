...
// how far across the dot is drawn, in CSS pixels. It is not attenuated by distance, so it
// is that size at every zoom, and a renderer scales a point by the pixel ratio - so it is
// that size on a screen of any density. Anything placed beside a dot or aimed at one is
// therefore placed and sized in CSS pixels as well.

const POINT_INDICATOR_SIZE = 32;
...
class PointIndicator extends Point {
	...
	constructor ( ctx, color ) {
		...
		const material = new PointsMaterial( { size: POINT_INDICATOR_SIZE, map: materials.pointerTexture, transparent : true, sizeAttenuation: false, alphaTest: 0.8, color: color } );
...
export { PointIndicator, POINT_INDICATOR_SIZE };